    ...
    try {
        ...
        if(params.downloadType != 'Status and Intensity') {
            ...
            while(tempEndDate.isBefore(endDate)) {
                ...
                headerWrote = (await createCsv(getObservationsServiceCall(params.downloadType), params, sheetName + requestTimestamp.toString() + '.csv', "observation", true, writeHeader, sites, individuals, observers, groups, phenophases, datasets))[1];
            ...
            csvFileNames.push((await createCsv(getObservationsServiceCall(params.downloadType), params, sheetName + requestTimestamp.toString() + '.csv', "observation", true, writeHeader, sites, individuals, observers, groups, phenophases, datasets))[0]);
        }
        else
            csvFileNames.push((await createCsv(getObservationsServiceCall(params.downloadType), params, 'status_intensity_observation_data' + requestTimestamp.toString() + '.csv', "observation", true, true, sites, individuals, observers, groups, phenophases, datasets))[0]);
        if(params.ancillary_data) {
            if(params.ancillary_data.indexOf("Sites") != -1)
                csvFileNames.push((await createCsv("stations/getStationDetails.json", { "ids": convertSetToArray(sites).toString(), 'no_live': true}, 'ancillary_site_data' + requestTimestamp.toString() + '.csv', "station", false, true, sites, individuals, observers, groups, phenophases, datasets))[0]);
            ...
            if(params.ancillary_data.indexOf("Protocols (7 files)") != -1) {
                ...
                csvFileNames.push((await createCsv("phenophases/getProtocolDetails.json", {}, 'ancillary_protocol_data' + requestTimestamp.toString() + '.csv', "protocol", false, true, sites, individuals, observers, groups, phenophases, datasets))[0]);
                csvFileNames.push((await createCsv("phenophases/getPhenophaseDetails.json", {}, 'ancillary_phenophase_data' + requestTimestamp.toString() + '.csv', "phenophase", false, true, sites, individuals, observers, groups, phenophases, datasets))[0]);
                csvFileNames.push((await createCsv("phenophases/getSecondaryPhenophaseDetails.json", {}, 'ancillary_species-specific_info_data' + requestTimestamp.toString() + '.csv', "sspi", false, true, sites, individuals, observers, groups, phenophases, datasets))[0]);
                csvFileNames.push((await createCsv("phenophases/getAbundanceDetails.json", {}, 'ancillary_intensity_data' + requestTimestamp.toString() + '.csv', "intensity", false, true, sites, individuals, observers, groups, phenophases, datasets))[0]);
                csvFileNames.push((await createCsv("observations/getDatasetDetails.json", {"dataset_id": convertSetToArray(datasets)}, 'ancillary_dataset_data' + requestTimestamp.toString() + '.csv', "dataset", false, true, sites, individuals, observers, groups, phenophases, datasets))[0]);
                csvFileNames.push((await createCsv("phenophases/getPhenophaseDefinitionDetails.json", {}, 'ancillary_phenophase_definition_data' + requestTimestamp.toString() + '.csv', "phenophase_definition", false, true, sites, individuals, observers, groups, phenophases, datasets))[0]);
            }
        }
